...
from summary import util
from summary.Segmentation import Segmentation
...
class TextRank4Keyword(object):
    ...
    def __init__(self, segmenter,
        ...
        self.seg = Segmentation(segmenter=segmenter, stop_words_file=stop_words_file,
    ...
    def analyze(self,
        ...
        result = self.seg.segment(text, lower=lower)
        self.sentences = result.sentences
        self.lexical_result = result.lexical_result
    ...
    def get_lexical_res(self):
        res = {'cws':'' , 'pos': '', 'ner': {'per': [], 'org': []}}
        for lexical_sentence in self.lexical_result:
            for w in lexical_sentence:
                res['cws'] += w.word + ' '
                res['pos'] += w.word + '|' + w.flag + '  '
                if w.flag.lower() == 'per':
                    res['ner']['per'].append(w.word)
                elif w.flag.lower() == 'org':
                    res['ner']['org'].append(w.word)
        res['ner']['per'] = list(set(res['ner']['per']))
        res['ner']['org'] = list(set(res['ner']['org']))
        return res
